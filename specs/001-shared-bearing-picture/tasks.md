...
- [X] T067 [US1] Render web service + Postgres; migrations on deploy — **live at `https://foxmapper.com` 2026-07-16**. Blueprint deploy, no code change; Postgres 18, migrations ran on first boot, `/health` green in ~8 min (cold Rust release build). One image serving the PWA and the API from the same origin. Custom domain set **before** any field work, because IndexedDB/`localStorage`/the service worker are per-origin and moving later would silently wipe every device's remembered hunt (FR-004c). Apex canonical, `www` 301s to it — the code gets read aloud, so syllables count. Route 53: apex is an **A to `216.24.57.1`** (its ALIAS only targets AWS resources, and CNAME is illegal at an apex)
...
- [X] T069 [US1] Verify T062's `curl -N` check **against the deployed URL**, not localhost — **passed, and re-run on `foxmapper.com` itself** so the check covers the origin the field test will use. Three reports posted 3 s apart arrived **158 / 119 / 136 ms** after their POSTs, 3.2 s apart — tracking the posts, one at a time, not a burst. SC-002 budgets 5 s; production delivers in ~140 ms. Also verified in prod: `id:` = server seq, `Last-Event-ID` resumes (catch-up and live push are one code path), `since=0` returns the whole hunt, unknown hunt streams `204`, HTTP/2 and forced HTTPS on the custom domain. The seq is **global across hunts, not per-hunt** — harmless, since the cursor only needs to be monotonic within a hunt and the stream filters by hunt. See [findings.md](findings.md)

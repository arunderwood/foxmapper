...
| Field | Type | Notes |
|-------|------|-------|
| `id` | string | Stable step identifier (e.g. `target`, `estimate`, `report-bar`, `bearing`, `omni`, `null`, `share`, `finish`). |
...
1. `target` → anchor `target-label` — what you're hunting (and `target-frequency` when known).
...
3. `report-bar` → anchor `report-bar` — "every radio helps"; intro to reporting.
4. `bearing` → anchor `report-bearing` — for a directional antenna / body-fade bearing.
5. `omni` → anchor `report-omni` — signal strength from any radio, incl. a stock handheld
   (the explicit stock-handheld message lives here, FR-008).
6. `null` → anchor `report-null` — "I hear nothing here" is evidence too.
7. `share` → anchor `share-hunt` — bring the team in; join needs no account/install/payment (FR-010).
8. `finish` → anchorless — land on a live hunt view, primed to report (FR-015); mentions relaunch.
